...
export default function LoginPage() {
  ...
  const [otp, setOtp] = React.useState(''); // Token input
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authService.sendForgotPasswordOtp(forgotEmail);
      
      // Parse message and display the reset token for direct local developer convenience
      let successMsg = response.message || 'Reset token generated successfully. Check your email.';
      if (response.data?.resetToken) {
        successMsg += ` Token: ${response.data.resetToken} (Copy this code for testing)`;
      }
      
      setActionSuccess(successMsg);
      setMode('RESET_PASS');
    } catch (err: any) {
      setActionError(err.message || 'Failed to dispatch password reset request.');
    } finally {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setActionSuccess('Password reset successfully. You can now log in with your new password.');
      setMode('LOGIN');
      // Clear forms
      setForgotEmail('');
      setOtp('');
      ...
      setActionError(err.message || 'Reset password verification failed. Please review the token.');
    } finally {
      setActionLoading(false);
    }
  };
              ...
              <div className="space-y-1">
                ...
                <div className="text-right px-2 mt-1">
                  <button
                    ...
                    className="text-[8px] sm:text-[9px] font-black text-rose-600 hover:text-rose-500 uppercase tracking-[0.1em] italic cursor-pointer focus:outline-none bg-transparent border-none"
                  >
                    Forgot Password?
                  </button>
                </div>
        ...
        {mode === 'FORGOT_OTP' && (
          <form onSubmit={handleRequestOtp} className="space-y-3 sm:space-y-4">
            ...
            <button
              ...
              className="w-full text-center text-[9px] sm:text-[10px] font-black text-slate-400 hover:text-slate-600 uppercase tracking-widest pt-2 cursor-pointer focus:outline-none bg-transparent border-none"
            >
              Back to Login
            </button>
          </form>
        ...
        {mode === 'RESET_PASS' && (
          <form onSubmit={handleResetPassword} className="space-y-3 sm:space-y-4">
            <div className="space-y-2 sm:space-y-3 text-left">
              ...
              <div className="space-y-1">
                <label htmlFor="otp-code" className="block text-[8px] sm:text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Reset Token (Code)</label>
                <div className="relative group">
                  <div className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-600 transition-colors">
                    <Lock className="w-4 h-4" />
                  </div>
              ...
              <div className="space-y-1">
                <label htmlFor="confirm-password" className="block text-[8px] sm:text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Confirm New Password</label>
                <div className="relative group">
                  <div className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-600 transition-colors">
                       <Lock className="w-4 h-4" />
                  </div>
