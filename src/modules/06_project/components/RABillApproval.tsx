...
import { ShieldAlert, ShieldCheck, Lock, Check, X, MessageSquare } from 'lucide-react';
import { UserRole } from '@/src/types';

interface RABillApprovalProps {
...
export default function RABillApproval({
  status,
  userRole,
  onApprove,
  onReject
  ...
  return (
    <div className="bg-amber-50/60 border border-amber-100 p-5 rounded-2xl space-y-4">
      <div className="flex gap-2.5 text-[10px] text-amber-800 font-bold uppercase leading-tight">
        ...
        <div>
          <p className="font-extrabold text-[10px] text-amber-850">Awaiting Client Sign-off & Audit</p>
          <p className="text-[9px] text-amber-600 mt-0.5 normal-case font-medium">
            This Running Account bill is representing public/private client receivables. SuperAdmin approval is required to certify that measurements align with on-site BOQ books.
          </p>
        </div>
      </div>
      ...
      {isSuperAdmin ? (
        <div className="space-y-3">
          {!showRemarksInput ? (
            <div className="flex gap-2.5">
              ...
              >
                <X className="w-3.5 h-3.5" /> Decline / Discrepancy
              </button>
              ...
              <button
                ...
                className="flex-1 py-2.5 px-3 text-[10px] font-black uppercase tracking-wider bg-indigo-650 hover:bg-indigo-700 text-white transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-sm"
              >
                <Check className="w-3.5 h-3.5" /> Pass Bill & Certify
              </button>
            </div>
          ) : (
            <form onSubmit={handleAction} className="space-y-3 animation-fade-in text-left">
              <div className="flex flex-col">
                <label className="text-[8px] font-black text-amber-800 uppercase tracking-wider mb-1 flex items-center gap-1">
                  <MessageSquare className="w-3 h-3 text-amber-500" />
                  Client Settlement Comments (Optional)
                </label>
                <textarea
                  value={remarks}
                  onChange={(e) => setRemarks(e.target.value)}
                  placeholder={`Include reason for this certification ${activeAction === 'Approve' ? 'authorization' : 'revision demand'}...`}
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className={`flex-1 py-2 px-3 text-[9px] font-black uppercase tracking-wider text-white rounded-lg transition-all cursor-pointer shadow-3xs flex items-center justify-center gap-1 ${
                    activeAction === 'Approve' ? 'bg-indigo-650 hover:bg-indigo-700 border border-indigo-750' : 'bg-rose-600 hover:bg-rose-700 border border-rose-750'
                  }`}
                >
                  Confirm Certification {activeAction === 'Approve' ? 'Pass' : activeAction}
                </button>
              </div>
            </form>
          )}
        ...
        <div className="flex items-center gap-1.5 justify-center py-2.5 text-[9px] text-slate-400 border border-dashed border-slate-200 bg-white rounded-xl font-black uppercase tracking-wider">
          <Lock className="w-3.5 h-3.5 text-slate-350" /> SuperAdmin Authentication Required For Sign-offs
        </div>
      )}
    </div>
  );
