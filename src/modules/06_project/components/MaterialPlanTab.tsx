...
export default function MaterialPlanTab({ items, onAdd, onDelete }: MaterialPlanTabProps) {
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/40">
              ...
              <th className="p-4 text-[9px] font-extrabold uppercase tracking-wider text-slate-400">Material Specification</th>
              <th className="p-4 text-[9px] font-extrabold uppercase tracking-wider text-slate-400 w-48">Demanded Resource Volume</th>
            ...
            {items.length > 0 ? (
              items.map((item, index) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4 text-xs font-black text-slate-800 uppercase flex items-center gap-2">
                    <Box className="w-3.5 h-3.5 text-slate-450" />
                    {item.name}
                  </td>
              ...
              <tr>
                <td colSpan={4} className="p-8 text-center text-slate-400 font-black text-2xs uppercase tracking-tight">
                  No materials mapped to this activity. Add a line below.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-50/30 p-4 rounded-xl border border-dashed border-slate-200/60 flex flex-col sm:flex-row items-end gap-3">
        <div className="flex-grow space-y-1">
          ...
          <input
            type="text"
            required
            placeholder="e.g. Portland Slag Cement (OPC 53)"
            value={name}
        ...
        <div className="sm:w-48 space-y-1">
          ...
          <input
            type="text"
            required
            placeholder="e.g. 150 bags, 25 tons"
            value={qty}
