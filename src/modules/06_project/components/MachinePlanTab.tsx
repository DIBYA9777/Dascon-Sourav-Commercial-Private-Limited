...
interface MachinePlanTabProps {
  items: ResourcePlanItem[];
  onAdd: (name: string, qty: string) => void;
  onDelete: (id: string) => void;
}

export default function MachinePlanTab({ items, onAdd, onDelete }: MachinePlanTabProps) {
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/40">
              ...
              <th className="p-4 text-[9px] font-extrabold uppercase tracking-wider text-slate-400">Machine / Heavy Equipment</th>
              <th className="p-4 text-[9px] font-extrabold uppercase tracking-wider text-slate-400 w-48">Demand Metric / Hours</th>
            ...
            {items.length > 0 ? (
              ...
              <tr>
                <td colSpan={4} className="p-8 text-center text-slate-400 font-black text-2xs uppercase tracking-tight">
                  No heavy machinery assigned to this activity node. Mapped equipment under are empty.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-50/30 p-4 rounded-xl border border-dashed border-slate-200/60 flex flex-col sm:flex-row items-end gap-3">
        <div className="flex-grow space-y-1">
          ...
          <input
            type="text"
            required
            placeholder="e.g. Excavator JCB 3DX / Concrete Transit Mixer / Soil Compactor"
            value={name}
        ...
        <div className="sm:w-48 space-y-1">
          ...
          <input
            type="text"
            required
            placeholder="e.g. 2 units (80 hrs), 1 unit"
            value={qty}
