...
import { UserPlus, Search, Filter, Mail, UserCheck, UserX, ShieldAlert, Trash2, Loader2 } from 'lucide-react';
  ...
  const handleDeleteUser = (id: string, role: UserRole) => {
    ...
    if (currentUser.role === UserRole.ADMIN) {
      if (role === UserRole.ADMIN) {
        alert("ACCESS DENIED: As an Admin, you do not have authority to delete other Administrators. Contact Super Admin.");
        return;
      }
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="overflow-hidden">
            <div className="flex items-center gap-2 flex-wrap mb-0.5">
              <span className="text-xs font-black bg-blue-600 text-white px-2 py-0.5 rounded-lg italic tracking-tight shadow-sm shrink-0">{item.userId || 'N/A'}</span>
    ...
    },
    {
      ...
      accessor: (item: User) => (
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={() => navigate('/admin/users')} className="text-xs h-9 px-3 font-black uppercase tracking-tight hover:bg-slate-50">Edit</Button>
          <Button 
            variant="ghost" 
            size="sm" 
          ...
          <Button 
            ...
            onClick={() => handleDeleteUser(item.id, item.role)}
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button onClick={() => navigate('/admin/users')} size="sm" className="shadow-lg shadow-blue-100 px-6 rounded-xl font-black italic uppercase text-xs">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-wrap gap-4 items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <select 
            className="bg-slate-50 border border-slate-200 rounded-lg py-2 px-3 text-sm outline-none focus:ring-2 focus:ring-blue-500 font-medium"
            value={roleFilter}
